...
function LoginPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) router.navigate({ to: "/shifts" });
    });
  }, [router]);
  ...
  return (
    <main className="safe-top safe-bottom min-h-screen grid place-items-center bg-background p-4">
      <div className="w-full max-w-sm">
        ...
        <Tabs defaultValue="signin" className="w-full">
          <TabsList className="grid w-full grid-cols-2 h-12">
            <TabsTrigger value="signin" className="h-10 text-base">Entrar</TabsTrigger>
            <TabsTrigger value="signup" className="h-10 text-base">Crear cuenta</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="signin" className="space-y-3 pt-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input id="si-pw" type="password" autoComplete="current-password" value={password}
                onChange={(e) => setPassword(e.target.value)} className="h-12 text-base" />
            </div>
